...
If you add a new CSS class that is not already defined, use `var()` for any colors that should be easy to customize. For example, the `banner_color` class in `assets/style.css` uses the `--banner-color` value defined in `assets/style_config_default.css`:
...
Then you can apply the class to a page. For example, the `banner_color` class was added to `index.qmd` by using:
